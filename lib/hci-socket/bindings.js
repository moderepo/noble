...
var CREATE_CONN_TIMEOUT = 10*1000;
...
NobleBindings.prototype._startConnection = function(peripheralUuid) {
  ...
  debug('create connection to %s', peripheralUuid);
  this._hci.createLeConn(address, addressType);
  
  this._pendingConnTimeout = setTimeout(() => {
    debug('cancel connection to %s', peripheralUuid);
    this._hci.cancelLeConn();
  }, CREATE_CONN_TIMEOUT);
};

NobleBindings.prototype.connect = function(peripheralUuid) {
  if (!this._pendingConnectionUuid) {
    this._startConnection(peripheralUuid);
  } else {
...
NobleBindings.prototype.onLeConnComplete = function(status, handle, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy) {
  debug('onLeConnComplete: status=%d handle=%s address=%s', status, handle, address);
  ...
  if (this._pendingConnTimeout) {
    clearTimeout(this._pendingConnTimeout);
    this._pendingConnTimeout = null;
  }
